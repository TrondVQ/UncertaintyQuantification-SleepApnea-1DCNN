...
2. Handles missing values in features by filling them with global column means.
3. Performs a patient-independent train/test split using GroupShuffleSplit
   to ensure no data from the same patient appears in both sets. The split is
   applied to the indices of the flattened data.
4. Reshapes the flattened 2D time-series data (samples, time_steps * features)
   into a 3D format (samples, time_steps, features) suitable for sequence models.
   This is done separately for the training and test sets after splitting.
5. Applies window-level standardization to the 3D data, standardizing each
   sample (window) independently along the time axis. This is applied to both
   the training and test sets after reshaping.
6. Applies balancing techniques:
   - SMOTE (Synthetic Minority Over-sampling Technique) is applied to the
     window-standardized and flattened training data to address class imbalance.
     The resulting data is then reshaped back to 3D.
   - Random Under-Sampling (RUS) is applied to the window-standardized and
     flattened test data to create a balanced test set for evaluating model
     performance on a balanced distribution, if both classes are present in
     the test set. The resulting data is then reshaped back to 3D.
7. Saves the resulting datasets as 3D NumPy arrays, using a naming convention
   that indicates the applied preprocessing steps:
   - SMOTE-balanced training set (`X_train_win_std_smote.npy`, `y_train_smote.npy`).
   - Unbalanced test set with window standardization (`X_test_win_std_unbalanced.npy`,
     `y_test_unbalanced.npy`). The corresponding patient IDs are also saved
     (`patient_ids_test_unbalanced.npy`).
   - RUS-balanced test set with window standardization, if successfully created
     (`X_test_win_std_rus.npy`, `y_test_rus.npy`).
...
def prepare_final_datasets(input_csv, output_dir, test_size, seed):
    ...
    try:
        ...
        np.save(os.path.join(output_dir, 'y_train_smote.npy'), y_train_smote)
